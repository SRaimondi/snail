    ...
    fn test_quaternion_from_matrix() {
        ...
        for _ in 0..TESTS {
            ...
            for _ in 0..ROTATIONS {
                ...
                assert!(q.rotate(Vec3f32::UNIT_X).approx_eq(s).all());
                assert!(q.rotate(Vec3f32::UNIT_Y).approx_eq(n).all());
                assert!(q.rotate(Vec3f32::UNIT_Z).approx_eq(t).all());
            }
        }
    }
}
